{ # all scripts will start and end with a curly bracket
  ...
  weatherData = read.csv(file="LansingNOAA2016.csv", 
                         sep=",",
                         header=TRUE, 
                         stringsAsFactors = FALSE);  
  
  # weatherData2 will be equivalent to weatherData if you are using R version 4
  weatherData2 = read.csv(file = "LansingNOAA2016.csv");
...
relHum = weatherData$relHum
minTemps =weatherData$minTemp
relHum274a = weatherData$relHum[274];
val78 = minTemps[78];
val362 = minTemps[362];
val276 = minTemps[276];
consecVals = relHum[341:366];
consecValsRev = minTemps[120:100];
seq1 = relHum[seq(from=1, to=3, by=1)];
relHum274b = weatherData$relHum
relHum274c = weatherData$relHum

dowJonesData = read.csv(file="DowJones1980_2012.csv",
                        sep="",
                        stringsAsFactors = FALSE);
